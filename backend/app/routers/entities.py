...
def create_goal(payload: GoalCreate, db: Db, user: CurrentUser) -> Goal:
    ...
    if payload.parent_goal_id:
        require_goal(db, user.id, payload.parent_goal_id)
...
def list_projects(db: Db, user: CurrentUser) -> list[dict]:
    project_ids = user_project_ids(db, user.id)
    if not project_ids:
        return []
    rows = db.execute(
...
def assign_task(task_id: int, payload: TaskAssign, db: Db, user: CurrentUser) -> Task:
    ...
    if payload.assignee_id is not None:
        ...
        is_owner = payload.assignee_id == item.project.owner_id
        is_member = db.scalar(
        ...
        if not is_owner and not is_member:
            raise HTTPException(status_code=403, detail="Assignee must be a project member")
...
def cast_vote(decision_id: int, payload: VoteCreate, db: Db, user: CurrentUser) -> Vote:
    ...
    if not decision:
        raise HTTPException(status_code=404, detail="Decision not found")
    if user.id != decision.author_id:
        if decision.goal_id:
            if decision.goal_id not in user_goal_ids(db, user.id):
                raise HTTPException(status_code=403, detail="Only goal participants can vote")
        else:
            raise HTTPException(status_code=403, detail="Only the decision author can vote")
...
@router.get("/decisions/{decision_id}/votes", response_model=VoteSummary)
def list_votes(decision_id: int, db: Db, user: CurrentUser) -> VoteSummary:
    decision = db.get(Decision, decision_id)
    if not decision:
        raise HTTPException(status_code=404, detail="Decision not found")
    if decision.author_id != user.id:
    ...
    own = next((v for v in votes if v.user_id == user.id), None)
    finalized = decision.status in ("accepted", "rejected")
    return VoteSummary(
        accept=sum(1 for v in votes if v.variant == "accept"),
        reject=sum(1 for v in votes if v.variant == "reject"),
        total=len(votes),
        quorum=decision.quorum,
        quorum_met=len(votes) >= decision.quorum,
        own_vote=VoteOut.model_validate(own) if own else None,
        votes=[VoteOut.model_validate(v) for v in votes] if finalized else None,
    )
