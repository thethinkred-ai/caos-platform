...
from ..models import AuditEvent, Decision, DecisionEvent, Goal, KnowledgeItem, Notification, Problem, Project, ProjectMember, Task, Team, TeamMember, User
from ..schemas import AuditEventOut, DecisionCreate, DecisionEventCreate, DecisionEventOut, DecisionOut, GoalCreate, GoalOut, MemberCreate, MemberOut, ProblemCreate, ProblemOut, ProjectCreate, ProjectMemberOut, ProjectOut, ProjectStatusUpdate, TaskAssign, TaskCreate, TaskOut, TeamCreate, TeamOut
...
def list_problems(db: Db) -> list[Problem]:
    return list(db.scalars(select(Problem).order_by(Problem.created_at.desc())))
...
def list_goals(db: Db) -> list[Goal]:
    return list(db.scalars(select(Goal).order_by(Goal.created_at.desc())))
...
def list_sub_goals(goal_id: int, db: Db) -> list[Goal]:
...
def list_audit_events(db: Db, user: CurrentUser) -> list[AuditEvent]:
